...
def account(username):
    ...
    comments= Comment.query.filter_by(user_id=user.id).order_by(Comment.date_posted.desc()).paginate(page=cpage,per_page=3)
    return render_template("account.html",user=current_user,img=image_file,page_user=user,posts = posts,comments=comments)
